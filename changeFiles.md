...
the --extended (-e) option (--extend works too!) with the command gives more information.
...
For OSM **change files always  use `--extended` (short -e)** with `osmium fileinfo` to
meaningful result.
